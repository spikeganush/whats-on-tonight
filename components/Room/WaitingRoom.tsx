...
export default function WaitingRoom({ room, users, isCreator, onStartGame }: WaitingRoomProps) {
    ...
    return (
        <View className="flex-1 bg-slate-900 p-6 w-full">
            ...
            <Text className="text-white text-xl font-semibold mb-4">Users Joined ({users.length})</Text>
            <FlatList 
                data={users}
                ...
                renderItem={({ item }) => (
                    <View className="bg-slate-800 p-4 rounded-xl mb-2 flex-row items-center">
                        <View className="w-8 h-8 rounded-full bg-indigo-500 items-center justify-center mr-3">
                            <Text className="text-white font-bold">{item.name[0]?.toUpperCase()}</Text>
                        </View>
                        <Text className="text-white text-lg">{item.name}</Text>
                        {item.sessionId === room.creatorId && <Text className="text-xs text-yellow-500 ml-2 border border-yellow-500 px-2 rounded">HOST</Text>}
                    </View>
                )}
            />
