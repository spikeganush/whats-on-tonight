 ...
 const overlaySuperStyle = useAnimatedStyle(() => ({
  ...
  return (
    <GestureDetector gesture={panGesture}>
      <Animated.View style={[styles.card, animatedStyle]} className="absolute bg-slate-800 rounded-3xl overflow-hidden shadow-xl border border-slate-700">
        <Image 
          source={{ uri: imageUrl }} 
          style={styles.image}
          resizeMode="cover"
        ...
        {index === 0 && (
            <>
                 <Animated.View style={[styles.overlay, { left: 40, transform: [{ rotate: '-30deg' }] }, overlayLikeStyle]}>
                    <Text className="text-green-500 border-4 border-green-500 font-bold text-4xl px-2 rounded-lg">LIKE</Text>
                 </Animated.View>
                 <Animated.View style={[styles.overlay, { right: 40, transform: [{ rotate: '30deg' }] }, overlayNopeStyle]}>
                    <Text className="text-red-500 border-4 border-red-500 font-bold text-4xl px-2 rounded-lg">NOPE</Text>
                 </Animated.View>
                  <Animated.View style={[styles.overlay, { alignSelf: 'center', top: '40%' }, overlaySuperStyle]}>
                    <Text className="text-blue-500 border-4 border-blue-500 font-bold text-4xl px-2 rounded-lg -rotate-12">SUPER</Text>
                 </Animated.View>
            </>
        )}
